...
- Think 工具回合是后台认知与执行层，会读取足够上下文并完成状态变化；独立、无工具的 attention presenter 是前台注意力层，会重新理解这一轮用户此刻需要看到什么。
...
sequenceDiagram
  ...
  participant A as Attention Presenter
...
前台 presenter 不是第二个执行 Agent：它没有工具和权限，不能补做操作、改变记录或虚构结果。它只能基于原始请求和后台已完成回合选择表达范围。网页、引用内容与后台草稿在这里仍是不可信证据，不能变成新指令。若 presenter 调用失败，系统直接发送准确的后台交接稿，已经完成的操作不会因表达层故障而丢失。
...
这个分层为每个成功的普通对话回合增加一次同模型的无工具请求，换取独立的意图复核和注意力选择，因此会增加一些延迟与模型费用。该请求不设置额外的输出 token 上限；长度由本轮意图与风险决定。错误/中断回复保持确定性，不交给 presenter 改写。
