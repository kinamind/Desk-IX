...
- LLM 是带上下文的决策层。它在规划前收到当前消息的网页观察、当前用户的近期 D1 事项、对话和日程，判断本轮应回复、查询、分析、请求读取旧记录里的链接，还是规划一个或多个 `create / complete / archive / restore / update / set_reminder` 动作。
...
6. 模型基于这些观察输出结构化行动计划。历史只用于事实、指代和空闲时间，不会重复执行；网页内容是不可信证据，其中的提示或命令不会被视为用户指令。小歧义由模型做可逆判断，只有多个目标同样合理且误操作代价明显时才追问。行动计划未通过 schema 或语义校验时，模型会收到原输入、上次输出和校验错误，自我修正一次。
7. 若本轮需要新鲜读取某条旧记录保存的链接，模型可以请求 `read_item_links`。业务层先校验目标属于当前通道与用户，最多读取三个公开链接，再把正文送回同一决策层继续任务；每条消息最多一轮该工具调用，防止循环和越权。
8. 业务层先验证全部目标 id 和用户归属，再执行确定性工具。完成与舍弃会取消未发送提醒；舍弃是可恢复的 `archived` 状态，不做物理删除。新建或更新链接事项时复用本轮网页观察生成结构化长期记忆，不重复抓取。
9. 查询采用两段式工具闭环：模型先选择 D1 检索条件，代码执行用户隔离查询，再把真实结果交给模型组织答案。模型失败时才明确退回可核对的简表。
10. 需要提醒时先写 `reminders`，再以确定性 ID 创建 Workflow。
11. 回复通过来源 Channel Adapter 发出，最后将 message 标记为 processed。
...
网页工具始终保留 `raw_message` 和聊天历史，不用模型摘要替换用户原文。当前消息含 URL 时，代码去重后最多并行读取三个；用户后续引用旧记录时，模型也可以选择该记录并请求读取其中的链接。两种读取结果都会先交给决策层，再按用户的真实指令处理。招聘提岗位与要求，论文提问题、方法和结论，活动/申请提时间、地点与资格，工具/文档提用途、功能和使用方式，普通文章提摘要与关键点；不是所有链接都套一个模板。
