...
`XHS_COOKIE` 是可选的小红书浏览器登录会话，不是用户名或密码。需要小红书读取时，按 [小红书读取与账号会话](xiaohongshu.md) 在 Dashboard 或 Wrangler 的隐藏输入中配置；不要写进 `wrangler.jsonc`、提交记录、日志或聊天消息。健康响应只显示 `integrations.xiaohongshu.configured` 布尔值。
...
5. 分享一张含多张配图的 QQ 小红书卡片，确认 Agent 使用原卡片链接读取正文，把全部可信配图交给当前模型分析，并更新同一条 raw 记录、保留正文与视觉来源状态；若会话或视觉分析失败，应明确降级，不能要求重发已有链接或编造图片文字。
6. 保存 Research Idea，确认没有自动生成研究方案。
7. 用口语建一个稍后处理的任务，确认模型没有选择当前时间，回复分别显示提醒时间与截止时间，并验证主动推送与“完成/稍后/改期/舍弃”。
8. 建一个带远期截止的 project，确认只安排模型明确选择的提醒，不会机械产生 30/7/1 天三个里程碑。
9. 建一个需要数小时、不能拖到截止前的任务，确认模型先读取日程，再创建若干互不重叠的实际工作时段；截止日期本身不能被伪装成忙碌时段。
10. 询问“我今天/这周有什么安排”，确认固定事件、截止、工作时段和提醒被明确区分，真实冲突会指出，提醒与截止不会被当成占用区间。
11. 要求“在未来一个月找一段连续两小时”，确认查询按该明确范围执行、返回合格空档，再由 Agent 结合档案选择，不退回固定 14 天或固定 14:00。
12. 让它把某天下午的多项安排整体后移并避开会议，确认先解析全部对象、整体检查冲突、逐项报告实际结果，不设置人为批量条数上限。
13. 在当前提醒附近告诉它“这个时间有事，晚一点再提醒我”，确认它修改原事项而不是新建重复待办，取消旧提醒，并将新提醒放到冲突窗口之后。
14. 紧接着说“刚才那个已经完成了”，确认原事项变为 completed、待提醒和未执行工作时段取消，并且没有生成重复记录。
15. 新建两条临时事项，再用一句话完成一条、舍弃另一条；随后用自然语言恢复舍弃项。
16. 发一句普通追问或闲聊，确认模型可以直接回复且不会自动写成 note/task。
17. 用两个不同授权身份设置不同的时区与 Daily Plan 时间，确认各自按本地时间发送且不会混入另一身份的事项。
18. 用非 allowlist 账号验证被拒绝。
19. 再次 `npm run deploy`，确认 D1 item、work session 与 Workflow reminder 未丢失。
