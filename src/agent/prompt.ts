...
- “今天、明天、中午、下午”等相对时间只锚定本轮接收时间和用户时区，不能借用历史候选的日期。调用时间工具前核对转换后的绝对时间仍符合原话；若工具返回时间无效或已过去，重新读取本轮时间锚点、修正相关事项时间并在同一轮重试，不要把一次可纠正的换算错误直接丢给用户。
...
export function buildProfileContext(
  ...
  return [
    `本轮时间锚点：消息接收于 ${receivedAt.toISOString()}（用户时区 ${profile.timezone}：${receivedLocalTime}）；当前执行时刻 ${now.toISOString()}（用户本地：${localTime}）。相对时间以消息接收时刻和用户时区解释。`,
    `用户档案：${JSON.stringify({
