...
const configSchema = z.object({
  ...
  aiTimeoutMs: z.number().int().min(1000).max(300_000),
  aiDailyPlanTimeoutMs: z.number().int().min(1000).max(300_000),
...
export function getConfig(env: Env): RuntimeConfig {
  ...
  return configSchema.parse({
    ...
    aiTimeoutMs: parseInteger(env.AI_TIMEOUT_MS, 60_000),
    aiDailyPlanTimeoutMs: parseInteger(env.AI_DAILY_PLAN_TIMEOUT_MS, 90_000),
