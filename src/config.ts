...
const configSchema = z.object({
  ...
  urlMaxBytes: z.number().int().min(16_384).max(8_388_608),
  mediaMaxBytes: z.number().int().min(262_144).max(25_000_000),
  xhsMaxBytes: z.number().int().min(262_144).max(4_000_000),
...
export function getConfig(env: Env): RuntimeConfig {
  ...
  return configSchema.parse({
    ...
    urlMaxBytes: parseInteger(env.URL_MAX_BYTES, 4_194_304),
    mediaMaxBytes: parseInteger(env.MEDIA_MAX_BYTES, 20_000_000),
    xhsMaxBytes: parseInteger(env.XHS_MAX_BYTES, 2_000_000),
